...
router.get('/', function (req, res) {
     latestModel.find(function (err, latests) {
         ...
         return res.render('sports',{ posts: latests,trendings:latests,category:"latest",randomImage:"lahore"});
     });
 });
 ...
 */
 router.get('/:id', function (req, res) {
     lat_detailModel.findOne({id: req.params.id}, function (err, latdetail) {
         if (err) {
             return res.status(500).json({
         ...
         return res.render('post',{ message: latdetail,category:"latest" , randomImage:"lahore"});
     });
 });
